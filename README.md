...
- マルチプレイの対応: プレイヤーごとに`ExecutingPlayer`インスタンスを生成し、マルチプレイ時にもデータが競合しないようにしました。

- 設定の外部化: データベース接続情報、制限時間やコマンド`/memorygame rank`による表示数、ブロックの間隔などはコードに埋め込まず、`config.yml`から読み込む仕様にしました。
