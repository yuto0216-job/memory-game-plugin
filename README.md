...
- スコアの永続化: スコアをデータベースに保存します(MyBatis + MySQL使用)
...
- 例外処理  
  ブロック破壊、ゲーム中のログアウト、二重起動などのケースに対して、
  ゲームが破綻しないよう明示的な処理を実装しました。

- マルチプレイ対応  
  プレイヤーごとに `ExecutingPlayer` インスタンスを生成し、
  複数人が同時にプレイしてもデータが競合しないようにしました。

- 設定の外部化  
  データベース接続情報、制限時間、`/memorygame rank` の表示件数、
  ブロックの間隔などはコードに埋め込まず、`config.yml` から読み込む仕様にしました。
